...
**E0j 起跑**：2026-09-21 11:37:00 启动（d6bd04f，dirty=0），能量门 RC=0、2-slab 门 RC=0，**生产段 11:42:13 开始**；run.log 回显 `jit=True` 与 `scaled(frozen)`。运行记录：桌面 `运行日志6-09-21_1142_MODE5_thermal-jaxCGjit_..._d6bd04f_voxel_pyamg_frozen_tjit_E0j.md`；查看 `bash ~/work/159/status_E0j.sh`。结果：见 11.15（通过，9.91 h）

### 11.15 E0j 结果：全高通过（2026-09-21 11:42 → 21:36，9.91 h）

**验收**（对 11.12 / E0j 记录 §0 的判据）：

| 判据 | 目标 | E0j 实际 | E0 |
|---|---|---|---|
| 跑完 | 11,885 步 + 冷却 + 释放 | **rc=0，全部完成**，48 个 vtu | 同 |
| 门禁 | 仅 `ledger_complete` 失败 | **10/11**，唯一失败 `ledger_complete`（fast 节奏固有） | 同 |
| 中途冻结 / 编译告警 | 0 | **0 / 0** | 0（E0 没中） |
| 未收敛的力学线性解 | ≈ 0 | **44**（全部在层 153–182，全部重建一次后收敛） | 303 |
| 建层级 | ≈ 182 | **226** = 182 模式变化 + 44 失败重建 | 485 |
| PARDISO 兜底 / jit 变体 / 容量增长 | 0 / 1 / 0 | 0 / 1 / 0 | 同 |
| 与 E1b 一致性 | u_max ≤ 1e-5、vm_max ≤ 1e-7、释放 ≤ 1e-6 | **7.9e-6 / 1.03e-7 / 3.4e-7**（596 个汇报点） | 8.7e-6 / 1.03e-7 / 7.7e-7 |
| 与 E0 一致性 | — | u_max ≤ 1.66e-5（185 个点 > 1e-5，集中在层 127–129）、vm_max ≤ 1.6e-8、T_max 逐位相同、释放 4.3e-7 | — |
| 总墙钟 | 明显 < 14.13 h | **9.91 h**（35,678.5 s，3.00 s/步） | 14.13 h |

**加速比**：对 E0 **1.426x**；对 E1b 1.803x——但 E1b 的热学路径是 eager 的，不再是同代码对照，PARDISO 参考解需用同一提交重跑（E1j，`0119-flash-voxel-fast-hybrid-tjit.json`）。
分段（每 1,560 步，E0j 对 E0）：1.15x → 1.26x → 1.41x → 1.52x → 1.63x → 1.60x → 1.29x。

**力学求解器**（`profile.json`，E0j ‖ E0）：调用 7,469 ‖ 7,469；CG 迭代 **815,679 ‖ 1,900,029**（−57%，平均 109 ‖ 244）；建层级 1,970 s ‖ 4,890 s；求解 6,314 s ‖ 14,071 s；
力学线性解合计 **2.30 h ‖ 5.27 h（2.29x）**；`frozen_scale_solves` 7,287。逐 10 层平均迭代：层 41–150 稳定在 86–124（E0 194–373），层 151–190 升到 100–267（E0 194–284）。

**44 次未收敛**：全部是复用层级、跑满 800 次、失败时相对残差中位 3.4e-6（最大 1e-2）；距上次建层级的位置集中在 4–10（即该层第 2–3 个力学步），失败前一次解已经要 486 次（中位）。
重建后 86–104 次收敛，无兜底。时间代价 249 s（跑满）+ 500 s（重建）≈ **12.5 min，占全程 2.1%**。
解读：零件顶部（层 153 以上）的切线漂移比中段更大，冻结缩放的层级在第 2–3 个力学步失效——正是 11.4 预言的「物理刚度变化大到预条件算子不定」的情形。
可选的修法已实现但本次关闭：`recovery="refresh"`（约 3 s 修复而不是 800 次 + 9 s 重建）与 §10.1.1 的走廊探测，合计能省约 8–10 min；论文里可作为消融项，不改变结论。

**内存**：主机 RSS 打印阶段中位 15.8 GB、最高 19.1 GB；释放解（PARDISO）HWM 31.6 GB；显存最高 11.8 GB；JAX 分配器峰值 7,689 MB（上限 13,858），全程平稳。

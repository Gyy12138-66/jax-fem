#!/usr/bin/env python3
"""Gate checks for the 159 (0119 part) runs: copy of check_cube_smoke.py with the flash energy
closure taken per physical layer (layer energies differ on a real part).
...
def main() -> None:
    ...
    if flash:
        ...
        layers_in_run = int(round(on_steps / float(flash["substeps"]))) if flash["substeps"] else 0
        per_layer = flash.get("physical_energy_per_layer_J")
        if isinstance(per_layer, list):
            physical_absorbed = absorptivity * sum(per_layer[:layers_in_run])
            capture_analytic = float(np.mean(ledger_pre["slab_capture_fraction"][:max(1, (layers_in_run + 4) // 5)]))
        else:
            physical_absorbed = absorptivity * float(per_layer) * layers_in_run
            capture_analytic = flash["capture_fraction_analytic"]
        energy["flash"] = {
            "capture_fraction_analytic": capture_analytic,
            "capture_fraction_ledger_over_analytic": (energy["capture_fraction"] / capture_analytic)

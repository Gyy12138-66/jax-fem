...
## 2. 阶段 1 结果（`output/v2_cube_smoke_smoke1/preflight`）
...
- 指纹：config `50844840…`，mesh `bde7f790…`，path `3bdde717…`，material `deae715a…`，contract `bee91250…`。
...
| 物理固相线下的生命周期事件 | 第一版 smoke（`v2_cube_smoke_smoke1/smoke`，solidus = liquidus = 1563 K）：峰温 1.65–1.98e3 K，几乎每步都有单元进出"熔态"→ 参考复位事件 → `install_mechanics_event_wrapper` 强制力学求解：实测 2.27 s/步（热-only 0.31），外推 ~10 h，且每次重扫同一 slab 都抹掉 eqp/T_ref。Balbaa 的立方体应力模型没有退火/重熔复位 | solidus = liquidus = 10000 K 哨兵（预检要求 ≥ 5000 K）；力学回到固定节拍；smoke1 在 step 300 处按 PID 停掉 |
...
门槛脚本 `check_cube_smoke.py`，结果写 `cube_smoke_gate.json`。**本节在 smoke 收尾后补写。**

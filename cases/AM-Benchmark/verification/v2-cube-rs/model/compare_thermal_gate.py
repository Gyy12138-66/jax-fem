...
def series_arrays(doc, key="avg_K", beam_bins_only=False):
    ...
    (Fable5 复审 2026-08-20 指出的措辞修正:这个条件是"采纳口径有效",
    **不是**"光束在圆内" —— 束心出圆之后热单元还会在阈值上残留约 7.7 ms,
    所以有效箱集合比"束在圆内"略宽。登记文本 D-V2-24 写的是对的,这里
    的旧措辞不准确;数值不受影响,筛选条件一直是 avg_K is not None。)

    这不是给双色路加阈值 —— 单元级仍然无阈值 —— 而是让三路踩在同一组采样
    时刻上。拍频造成的曝光量另由 `aliasing_envelope` 如实上报,不藏进插值里。
    """
    t, v = [], []
    for s in doc["series"]:
